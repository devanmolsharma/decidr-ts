...
function imageRow(state: ContentBlock[]): Row {
  return {
    id: "r1", state, question: "what is it?",
    options: [{ id: "cat", description: "a cat" }, { id: "dog", description: "a dog" }],
  };
}

test("validateRow: accepts content-block state with url image", () => {
  assert.doesNotThrow(() =>
    validateRow(imageRow([{ type: "text", text: "look:" }, { type: "image", url: "https://example.com/cat.png" }])),
  );
});

test("validateRow: accepts content-block state with inline data image", () => {
  assert.doesNotThrow(() => validateRow(imageRow([{ type: "image", data: "aGVsbG8=", mimeType: "image/png" }])));
});
...
test("validateRow: rejects an image block with both url and data", () => {
  assert.throws(() => validateRow(imageRow([{ type: "image", url: "https://x/y.png", data: "aGk=" }])), DecisionError);
});

test("validateRow: rejects an unknown block type", () => {
  assert.throws(() => validateRow(imageRow([{ type: "bogus" } as unknown as ContentBlock])), DecisionError);
});
...
test("Client.decide: works end-to-end with a multimodal row via a fake backend", async () => {
  ...
  const backend = new FakeBackend((messages) => {
    ...
    if (Array.isArray(user.content) && user.content.some((b) => b.type === "image")) sawImageBlock = true;
    return singleTokenReply("cat", -0.1, [["dog", -2.0]]);
  });
  const client = new Client("test-model", { backend, cache: false });
  const decision = await client.decide(imageRow([{ type: "image", url: "https://example.com/cat.png" }]));
