import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { EXAMPLE_PRESETS, type ExamplePreset } from "@/lib/examples";

function StarIcon() {
  return (
    <svg viewBox="0 0 16 16" width="12" height="12" fill="currentColor" aria-hidden="true">
      <path d="M8 .25a.75.75 0 0 1 .673.418l1.882 3.815 4.21.612a.75.75 0 0 1 .416 1.279l-3.046 2.97.719 4.192a.75.75 0 0 1-1.088.791L8 12.347l-3.766 1.98a.75.75 0 0 1-1.088-.79l.72-4.194L.818 6.374a.75.75 0 0 1 .416-1.28l4.21-.611L7.327.668A.75.75 0 0 1 8 .25Z" />
    </svg>
  );
}

/** The real GitHub mark, not just a star -- makes it unambiguous at a
 * glance that these header buttons are links to a GitHub repo, not just
 * a rating/favorite control. */
function GitHubIcon() {
  return (
    <svg viewBox="0 0 16 16" width="13" height="13" fill="currentColor" aria-hidden="true">
      <path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.01 8.01 0 0 0 16 8c0-4.42-3.58-8-8-8Z" />
    </svg>
  );
}

/** A thin app-shell header: branding, links to both repos, and the
 * example picker. Provider/model/API key selection lives down in the
 * Questions panel next to Run request instead (see ModelPicker) -- it's
 * an input to that one action, not global page chrome, and keeping it
 * here crowded the header on smaller widths. */
export function TopBar({ activePresetId, onLoadPreset }: { activePresetId: string; onLoadPreset: (preset: ExamplePreset) => void }) {
  return (
    <header className="h-14 shrink-0 border-b border-border px-5 flex items-center gap-3 overflow-x-auto">
      <span className="font-mono font-bold text-[14px] tracking-tight text-foreground shrink-0">decidr-ts</span>
      <a
        href="https://github.com/devanmolsharma/decidr-ts"
        target="_blank"
        rel="noreferrer"
        className="shrink-0 flex items-center gap-1.5 text-[11px] font-mono text-muted-foreground hover:text-foreground border border-border rounded-full px-2 h-6 transition-colors"
        title="decidr-ts on GitHub -- star the repo"
      >
        <GitHubIcon />
        decidr-ts
        <StarIcon />
      </a>
      <a
        href="https://github.com/devanmolsharma/decidr"
        target="_blank"
        rel="noreferrer"
        className="shrink-0 flex items-center gap-1.5 text-[11px] font-mono text-muted-foreground hover:text-foreground border border-border rounded-full px-2 h-6 transition-colors"
        title="decidr (Python port) on GitHub -- star the repo"
      >
        <GitHubIcon />
        decidr (Python)
        <StarIcon />
      </a>

      <div className="flex-1 min-w-3" />

      <span className="text-[11px] font-mono uppercase tracking-wider text-signal shrink-0">Try an example</span>
      <Select value={activePresetId} onValueChange={(id) => onLoadPreset(EXAMPLE_PRESETS.find((p) => p.id === id)!)}>
        <SelectTrigger className="w-auto min-w-52 h-8 text-[13px] shrink-0 rounded-full border-signal/50 bg-signal/10 text-foreground font-medium hover:bg-signal/15">
          {/* base-ui's SelectValue falls back to the raw id (e.g.
           * "ticket-routing") without a children render-function to
           * resolve the real title from -- see ModelPicker.tsx's
           * provider select for the same fix and the full explanation. */}
          <SelectValue>{() => EXAMPLE_PRESETS.find((p) => p.id === activePresetId)?.title ?? activePresetId}</SelectValue>
        </SelectTrigger>
        <SelectContent className="max-w-80">
          {EXAMPLE_PRESETS.map((preset) => (
            <SelectItem key={preset.id} value={preset.id} className="text-[13px]">
              <span className="whitespace-normal">{preset.title}</span>
            </SelectItem>
          ))}
        </SelectContent>
      </Select>
    </header>
  );
}
